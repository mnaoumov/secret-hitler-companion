 ...
 *
 * This repo resolves no dependency on `obsidian-dev-utils` - it is scaffolded from `typescript-template`,
 * not a consumer of that library - so the custom rules it lints itself with are hand-copied out of that
 * repo's
 ...
 * moving underneath a tree nobody touches is invisible to a hook. Both halves really do fire here: husky is
 * wired up (`core.hooksPath` is `.husky/_`, and `.husky/pre-commit` runs `nano-staged`) and the CI step
 * below it runs on every push and pull request. That is the shape `typescript-template` carries and the one
 * a repo scaffolded from it inherits.
 *
